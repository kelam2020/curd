...
function getTotal(){/*تشتغل لما اكتب على اي انبوت من price taxes ads discount منحط اونكياب في اتش تمل*/
    ...
    if(price.value !=''){//هيك ما ح يشتغل الكود غير لما نعبي البرايس
        let result = (+price.value * +taxes.value) || (+price.value *+ +ads.value);
...
function updateData(i)//العنصر يلي بيتعدل
{
  ...
  history.value = datapro[i].history;
